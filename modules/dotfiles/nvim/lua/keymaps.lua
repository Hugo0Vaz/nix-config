...
vim.keymap.set({ "n", "i", "s" }, "<C-h>", function()
	...
	if ok and luasnip.jumpable(-1) then
		...
		vim.cmd("normal! <C-w><C-h>")
	end
end, { desc = "Jump to previous snippet node or move focus left" })
...
vim.keymap.set({ "n", "i", "s" }, "<C-l>", function()
	...
	if ok and luasnip.jumpable(1) then
		...
		vim.cmd("normal! <C-w><C-l>")
	end
end, { desc = "Jump to next snippet node or move focus right" })
